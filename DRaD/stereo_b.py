...
def reduce(timeframe, cadence):
    ...
    dir_plastic = root_dir + '/plastic/level2/Protons/Derived_from_1D_Maxwellian/behind'
    ...
    for day in date_range:
        ...
        # Get coordinates
        #print('between here')
        carr_lons, r, lats, lon = suppress_output(get_coordinates.get_coordinates, coord_df, 'behind')
    ...
    stereo_b_df = pd.concat([coords_df, plastic_df, mag_df], axis=1)

    stereo_b_df['P_t'] = (stereo_b_df['N'] * stereo_b_df['V']**2) / 10**19 / 1.6727   * 10**6 *10**9 # J/cm^3 to nPa
    stereo_b_df['P_B'] = stereo_b_df['B']**2 / 2. / 1.25663706212*10**(-6) / 10**9    * 10**6 *10**9 #nT to T # J/cm^3 to nPa
    stereo_b_df['P'] = stereo_b_df['P_t'] + stereo_b_df['P_B']
    stereo_b_df['Beta'] = stereo_b_df['P_t'] / stereo_b_df['P_B']
    stereo_b_df['POL'] = np.sign(stereo_b_df['B_R'] - stereo_b_df['B_T']*stereo_b_df['R']*2.7*10**(-6)/stereo_b_df['V'])
    stereo_b_df['S_P'] = stereo_b_df['T']/stereo_b_df['N']**(2./3.)/11604.5

    stereo_b_df['Spacecraft_ID'] = 5


    for col in stereo_b_df.columns:
        stereo_b_df.loc[stereo_b_df[col] < -1000, col] = np.nan

    return stereo_b_df


def plot(stereo_b_df):
    ...
    try:
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='V', ax=axes[0], color='black')
    except KeyError:
        pass
    axes[0].set_ylabel('$[km s^{-1}]$')

    try:
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='N', ax=axes[1], color='black')
    except KeyError:
        pass
    axes[1].set_ylabel('N $[cm^{-3}]$')

    try:
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='P', ax=axes[2], color='black')
    except KeyError:
        pass
    axes[2].set_ylabel('P $[nPa]$')

    try:
        stereo_b_df['polarity'] = ['+' if pol > 0 else '-' for pol in stereo_b_df['POL']]
        colors = {'-': 'tab:blue', '+': 'tab:red'}
        sns.scatterplot(data=stereo_b_df, x=stereo_b_df.index, y='B', ax=axes[3], hue='polarity', palette=colors, s=5, alpha=1)
    except KeyError:
        pass
    axes[3].set_ylabel('B $[nT]$')

    try:
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='B_R', ax=axes[4], color='red', label='B_R')
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='B_T', ax=axes[4], color='green', label='B_T')
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='B_N', ax=axes[4], color='blue', label='B_N')
    except KeyError:
        pass
    axes[4].set_ylabel('B $[nT]$')

    try:
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='S_P', ax=axes[5], color='black')
    except KeyError:
        pass
    axes[5].fill_between(stereo_b_df.index, 2.69, 4, color='grey', alpha=0.7)
    axes[5].set_ylabel('$S_p$ $[eV cm^{2}]$')

    try:
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='R', ax=axes[6], color='black')
    except KeyError:
        pass
    ...
    try:
        ax2 = axes[1].twinx()
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='T', ax=ax2, color='tab:blue')
        ax2.set_ylabel('T $[K]$')
    ...
    try:
        ax5 = axes[6].twinx()
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='LAT', ax=ax5, color='tab:blue')
        ax5.set_ylabel('LAT $[°]$')
    ...
    try:
        ax3 = axes[2].twinx()
        sns.lineplot(data=stereo_b_df, x=stereo_b_df.index, y='Beta', ax=ax3, color='tab:blue')
...
def load(month):
    ...
    root_dir = 'reduced_data/stereo_b'
...
def download_reduce_save_space(month, cadence):
    ...
    from DRaD import stereo_b, filefinder, utils
    import os
    import matplotlib.pyplot as plt
    ...
    for m in month:
        ...
        if os.path.exists('reduced_data\stereo_b\stereo_b_data'+m+'.parquet'):
            stereo_b_df = stereo_b.load(m)

        else:
            timeframe = filefinder.get_month_dates(m)

            stereo_b.download(timeframe)
            stereo_b_df = stereo_b.reduce(timeframe, cadence)
            stereo_b_df.to_parquet('reduced_data\stereo_b\stereo_b_data'+m+'.parquet')

        try:
            # Plot and save the figure
            utils.plot_timeseries(stereo_b_df)
            plt.savefig(f'stereo_data/monthly_plots/stereo_b_{m}.png')
            plt.close()  # Close the plot to free up memory
        except Exception as e:
            print(f"Error plotting data for {m}: {e}")
        finally:
            # Ensure stereo_b.delete() is called regardless of success or failure
            stereo_b.delete(m)